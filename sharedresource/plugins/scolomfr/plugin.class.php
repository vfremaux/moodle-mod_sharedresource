...
class sharedresource_plugin_scolomfr extends sharedresource_plugin_base {
	...
	var $METADATATREE = array(
	    ...
	    '1_10' => array(
		'name' => 'Type général de documents',
		'source' => 'scolomfr',
		'type' => 'select',
...
'schéma/graphique', 'site web', 'tableau (art)', 'web média'),
		...
		'source' => 'lom',
		'type' => 'select',
		'values' => array('exercise', 'annales', 'simulation', 'questionnaire', 'diagram', 'cyberquête', 'étude de cas', 'jeu éducatif', 'figure', 'graph', 'index', 'slide', 'table', 'narrative text', 'exam', 'experiment', 'problem statement', 'self assessment', 'lecture', 'manuel d\'enseignament', 'production d\'élève', 'démonstration', 'animation', 'tutoriel', 'glossaire', 'guide', 'matériel de référence', 'méthodologie', 'outil', 'scénario pédagogique', 'méthode de langues', 'témoignage pédagogique'),
		'checked' => array(
					'system'  => 1,
					'indexer' => 1,
					'author'  => 0,
		...
		'source' => 'scolomfr',
		'type' => 'select',
		'values' => array('en amphithéâtre', 'en atelier', 'en atelier de pédagogie personnalisée', 'en CDI', 'en salle de classe', 'en établissement', 'espace dédié à une pratique spécifique', 'en établissement socioculturel', 'en bibliothèque médiathèque', 'en mobilité', 'en musée',
'hors établissement', 'en installation de loisirs', 'en installation sportive', 'en laboratoire', 'en laboratoire de langues', 'en milieu familial', 'en milieu professionnel', 'en entreprise', 'non précisé', 'en salle informatique', 'en salle multimédia'),
		'checked' => array(
					'system'  => 1,
					'indexer' => 1,
					'author'  => 0,
		...
		'source' => 'scolomfr',
		'type' => 'select',
		'values' => array('à distance','en alternance', 'en autonomie', 'en classe entière', 'en collaboration','en milieu professionnel', 'en groupe', 'en groupe de compétences', 'en ligne', 'en tutorat', 'modalité mixte', 'séjour pédagogique', 'sortie pédagogique', 'travail de recherche', 'travail en interdisciplinarité', 'travaux dirigés', 'travaux pratiques'),
		'checked' => array(
					'system'  => 1,
					'indexer' => 1,
					'author'  => 0,
    ...
    function sharedresource_entry_definition(&$mform){
    	...
    	foreach(array_keys($this->METADATATREE['0']['childs']) as $fieldid){
    		...
    		if (get_config('sharedresource_scolomfr', $metadataswitch)){
    			$fieldtype = $this->METADATATREE['0']['childs'][$fieldid];
    			$generic = $this->METADATATREE[$fieldid]['name'];
    			if ($fieldtype == 'list'){
    				if ($instances = get_records_select('sharedresource_metadata', " entry_id = '{$this->entryid}' AND namespace = 'lomfr' AND name LIKE '$generic:%' ")){
    					$iterators[] = 0;
    ...
    function sharedresource_entry_definition_rec(&$mform, $nodeid, &$iterators){
		...
		if ($this->METADATATREE[$nodeid]['type'] == 'category'){
			...
			$elementinstances = get_records_select('sharedresource_metadata', " entry_id = '{$this->entryid}' AND namespace = 'lomfr' and name LIKE '{$generic}:%' " );
			// iterate on instances
    		$metadataswitch = $mform->metadataswitch.$nodeid;
			if ($instances && get_config('sharedresource_scolomfr', $metadataswitch)){
				$iterators[] = 0;
				foreach($instances as $instance){
	    			$this->sharedresource_entry_definition_rec($mform, $fieldid, $iterators);
    ...
    */
    function sharedresource_entry_definition_taxum(&$mform, $table, $idfield, $entryfield, $context){
    	...
    	if (empty($idfield) || empty($entryfield)){    	
	    	$optionsrec = get_records_select($table, "$context", "$idfield, $entryfield", "$idfield");
    ...
    */
    function configure($config){
		...
		echo '<legend><b>&nbsp;'.get_string('scolomfrformat', 'sharedresource').'</b></legend>';
		echo "<br/><center>";
    ...
    */
    function print_configure_rec($fieldid, $parentnode = '0'){
		...
		$checked_system = (get_config('sharedresource_scolomfr', "config_scolomfr_system_{$fieldid}")) ? 'checked="checked"' : '' ;
		$checked_indexer = (get_config('sharedresource_scolomfr', "config_scolomfr_indexer_{$fieldid}")) ? 'checked="checked"' : '' ;
		$checked_author = (get_config('sharedresource_scolomfr', "config_scolomfr_author_{$fieldid}")) ? 'checked="checked"' : '' ;
		...
		if($parentnode == '0'){
			echo "<td width='15%' align='center'><input id=\"lomfr_system_{$fieldid}\" type=\"checkbox\" name=\"config_scolomfr_system_{$fieldid}\" $checked_system value=\"1\" onclick=\"toggle_childs('lomfr', 'system', '{$fieldid}')\" /></td>";
			echo "<td width='15%' align='center'><input id=\"lomfr_indexer_{$fieldid}\" type=\"checkbox\" name=\"config_scolomfr_indexer_{$fieldid}\" $checked_indexer value=\"1\" onclick=\"toggle_childs('lomfr', 'indexer', '{$fieldid}')\" /></td>";
			echo "<td width='15%' align='center'><input id=\"lomfr_author_{$fieldid}\" type=\"checkbox\" name=\"config_scolomfr_author_{$fieldid}\" $checked_author value=\"1\" onclick=\"toggle_childs('lomfr', 'author', '{$fieldid}')\" /></td>";
			if(isset($field['widget'])){
				echo "<td width='15%' align='center'><input id=\"lomfr_widget_{$fieldid}\" type=\"checkbox\" name=\"widget_scolomfr_{$fieldid}\" $checked_widget value=\"1\"/></td></tr>";
			} else {
				echo "<td width='15%' align='center'></td></tr>";
			}
		} else {
			if($checked_system == 'checked="checked"'){
				echo "<td align='center'><input id=\"lomfr_system_{$fieldid}\" type=\"checkbox\" name=\"config_scolomfr_system_{$fieldid}\" $checked_system value=\"1\" onclick=\"toggle_childs('lomfr', 'system', '{$fieldid}')\"/></td>";
			} else {
				echo "<td align='center'><input id=\"lomfr_system_{$fieldid}\" type=\"checkbox\" name=\"config_scolomfr_system_{$fieldid}\" $checked_system value=\"1\" onclick=\"toggle_childs('lomfr', 'system', '{$fieldid}')\" DISABLED/></td>";
			}
			if($checked_indexer == 'checked="checked"'){
				echo "<td align='center'><input id=\"lomfr_indexer_{$fieldid}\" type=\"checkbox\" name=\"config_scolomfr_indexer_{$fieldid}\" $checked_indexer value=\"1\" onclick=\"toggle_childs('lomfr', 'indexer', '{$fieldid}')\" /></td>";
			} else {
				echo "<td align='center'><input id=\"lomfr_indexer_{$fieldid}\" type=\"checkbox\" name=\"config_scolomfr_indexer_{$fieldid}\" $checked_indexer value=\"1\" onclick=\"toggle_childs('lomfr', 'indexer', '{$fieldid}')\" DISABLED/></td>";
			}
			if($checked_author == 'checked="checked"'){
				echo "<td align='center'><input id=\"lomfr_author_{$fieldid}\" type=\"checkbox\" name=\"config_scolomfr_author_{$fieldid}\" $checked_author value=\"1\" onclick=\"toggle_childs('lomfr', 'author', '{$fieldid}')\"/></td>";
	
			} else {
				echo "<td align='center'><input id=\"lomfr_author_{$fieldid}\" type=\"checkbox\" name=\"config_scolomfr_author_{$fieldid}\" $checked_author value=\"1\" onclick=\"toggle_childs('lomfr', 'author', '{$fieldid}')\" DISABLED/></td>";
			}
			if(isset($field['widget'])){
				if($checked_widget == 'checked="checked"'){
					echo "<td align='center'><input id=\"lomfr_widget_{$fieldid}\" type=\"checkbox\" name=\"widget_scolomfr_{$fieldid}\" $checked_widget value=\"1\"/></td></tr>";
    ...
    function search(&$fromform, &$result) {
        ...
        $selectsql  = '';
        $selectsqlor  = '';
        $selectsql .= $CFG->prefix.'sharedresource_entry WHERE (';
        $selectsqlor    = '';
        ...
        if ($fromform->title || $fromform->description) {
            ...
            if ($fromform->search == '*') {
                $resources = get_records('sharedresource_entry', null, $sort);	// A VERIFIER !!!
            } else {
				$sql = 'SELECT * FROM '. $selectsql .' ORDER BY '. $sort;
                $resources = get_records_sql($sql, null, $page, $recordsperpage); // A VERIFIER !!!
            }
        }
        // append the results
        if (!empty($resources)) {
    ...
    */
    function get_metadata(&$sharedresource_entry, $namespace = null){
        ...
        $fields = get_records_select('sharedresource_metadata', " entry_id = '$sharedresource_entry->id' AND namespace = '$namespace' ");
	...
	*/
    function setKeywords($keywords){
    	...
    	delete_records_select('sharedresource_metadata', " namespace = 'lomfr' AND element LIKE '1_5:0_%' AND entry_id = {$this->entryid} ");
    	if ($keywordsarr = explode(',', $keywords)){
	    	...
	    	foreach($keywordsarr as $aword){
	    		...
	    		$mtdrec->value = addslashes($aword);
	    		insert_record('sharedresource_metadata', $mtdrec);
	    		$i++;
	    	}
	    }
    }
	...
	*/
    function setTitle($title){
		...
		delete_records('sharedresource_metadata', 'entry_id', $this->entryid, 'namespace', 'lomfr', 'element', '1_2:0_0');
		
		$mtdrec = new StdClass;
		$mtdrec->entry_id = $this->entryid;
		$mtdrec->element = '1_2:0_0';
		$mtdrec->namespace = 'lomfr';
		$mtdrec->value = addslashes($title);

		return insert_record('sharedresource_metadata', $mtdrec);
    }
	...
	*/
    function setDescription($description){
		...
		delete_records('sharedresource_metadata', 'entry_id', $this->entryid, 'namespace', 'lomfr', 'element', '1_4:0_0');

		$mtdrec = new StdClass;
		$mtdrec->entry_id = $this->entryid;
		$mtdrec->element = '1_4:0_0';
		$mtdrec->namespace = 'lomfr';
		$mtdrec->value = addslashes($description);
