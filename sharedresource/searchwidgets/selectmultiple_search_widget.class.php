...
class selectmultiple_search_widget extends search_widget{
    ...
    */
    function print_search_widget($layout, $value = 0) {
		...
		helpbutton('selectsearch', get_string('selectsearch', 'sharedresource'), 'sharedresource');
		echo '</td><td>';
		...
		foreach($mtdstandard->METADATATREE[$this->id]['values'] as $optvalue) {
			$selected = ($this->checkvalue($optvalue, $value)) ? 'selected="selected"' : '';
			echo '<option value="'.$optvalue.'">'.get_string(clean_string_key($optvalue), 'sharedresource').'</option>';
		}
