<?php
	...
	require_js($CFG->wwwroot.'/mod/sharedresource/js/taxon.js');
	...
	if($classifarray[$classification]['restriction'] == ''){
		$classtable = get_records($classification);
	} else {
		$classtable = get_records_sql($classifarray[$classification]['restriction']);
	}
	$error = '';
	$updatecomplete= '';
	...
	echo '<center>';
	echo '<form name="classifmodif" action="classificationconfigure2.php?classification='.$classification.'&mode=modif" method="post">';
	echo '<fieldset id="ClassList" style="auto;width:75%;">';
	echo '<legend align="center">';
	echo get_string('classificationmodif','sharedresource');
	helpbutton("modifclassification", 'modifclassification', "sharedresource");
	echo '</legend><br/>';
	if($error != ''){
	...
	echo '</td><td><input name="id" size="50" value="'.$classifarray[$classification]['id'].'"/></td>';
	echo '</tr><tr>';
	echo '<td align="center" width="200px">';
	echo get_string('parentname','sharedresource');
	echo '</td><td><input name="parent" size="50" value="'.$classifarray[$classification]['parent'].'"/></td>';
	echo '</tr><tr>';
	echo '<td align="center" width="200px">';
	echo get_string('labelname','sharedresource');
	echo '</td><td><input name="label" size="50" value="'.$classifarray[$classification]['label'].'"/></td>';
	echo '</tr><tr>';
	echo '<td align="center" width="200px">';
	echo get_string('orderingname','sharedresource');
	echo '</td><td><input name="ordering" size="50" value="'.$classifarray[$classification]['ordering'].'"/></td>';
	echo '</tr><tr>';
	echo '<td align="center" width="200px">';
	...
	echo '</SELECT></td>';
	echo '</table><br/>';
	echo '<center><input type="submit" value="'.get_string('modificationbutton','sharedresource').'"/></center>';
	echo '</fieldset><br/>';
	echo '</form>';
	echo '<fieldset id="ClassSelect" style="margin:0 auto;width:75%;">';
	echo '<legend align="center">';
	echo get_string('taxonchoicetitle','sharedresource');
	helpbutton("selecttaxon", 'selecttaxon', "sharedresource");
	echo '</legend><br/>';
	echo '<center>';
