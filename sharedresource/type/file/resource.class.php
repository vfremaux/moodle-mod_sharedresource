...
*/
class sharedresource_file extends sharedresource_base {

    function shaedresource_file($cmid=0, $identifier=false) {
        parent::sharedresource_base($cmid, $identifier);
    }
...
//                                           'value'   => $this->set_encrypted_parameter()),
    ...
    /** 
    * add an instance of a file based
    */
    ...
    */
    ...
    /// Set up some shorthand variables
        ...
        $sharedresource_entry = sharedresource_entry::get_by_identifier($resource->identifier);

        // if we dont get the resource then bail
        if (!$sharedresource_entry) {
            sharedresource_not_found($course->id);
        }
        $resource->reference = (!empty($sharedresource_entry->file)) ? $sharedresource_entry->file : $sharedresource_entry->url;
        ...
        if (sharedresource_is_url($resource->reference)) { // shared resource is a pure URL
            ...
            $fullurl = sharedresource_get_file_url(SHAREDRESOURCE_RESOURCEPATH.$resource->reference, $querys);
        }
        ...
        if (isset($resource->popup) && $resource->popup and !$inpopup) {    /// Make a page and a pop-up window
            ...
            print_header($pagetitle, $course->fullname, $navigation,
                    '', '', true, update_module_button($cm->id, $course->id, $this->strsharedresource), navmenu($course, $cm));
        ...
        add_to_log($course->id, 'sharedresource', 'view', "view.php?identifier={$resource->identifier}", $resource->title, @$this->cm->id);
        ...
        if ($embedded) {       // Display resource embedded in page
            ...
            if ($inpopup) {
                ...
                print_header_simple($pagetitle, '', $navigation, '', '', true,
                    update_module_button($cm->id, $course->id, $this->strsharedresource), navmenu($course, $cm, 'self'));
    ...
    function set_encrypted_parameter() {
        ...
        if (!empty($this->resource->reference) && file_exists($CFG->dirroot .'/mod/sharedresource/type/file/externserverfile.php')) {
            include $CFG->dirroot .'/mod/sharedresource/type/file/externserverfile.php';
            if (function_exists(extern_server_file)) {
                return extern_server_file($this->resource->reference);
            }
        }
        return md5($_SERVER['REMOTE_ADDR'].$CFG->sharedresource_secretphrase);
    }
    ...
    /**
     * Adds any "file resource" additional fields to the activity setup form
     */
    function setup_elements(&$mform) {
        global $CFG, $USER, $SHAREDRESOURCE_WINDOW_OPTIONS;
        ...
        $insertinpage  = optional_param('insertinpage', 0, PARAM_INT);
        if (!empty($add)) {
            $entry_id = optional_param('entry_id', false, PARAM_INT);
            // Have we selected a resource yet ?
            if (empty($entry_id)) {
                redirect($CFG->wwwroot.
                  "/mod/sharedresource/search.php?course={$course}&section={$section}&type={$type}&add={$add}&return={$return}&insertinpage={$insertinpage}");
            }
            // we have our reference Shared resource
            else {
                if (!$sharedresource_entry = sharedresource_entry::read_by_id($entry_id)) {
                    error('Invalid sharedresource_entry::id supplied: '.$entry_id);
                }
            }
        }
            ...
            if (!$sharedresource_entry = sharedresource_entry::read($resource->identifier)) {
                error('Invalid sharedresource_entry::identifier supplied: '.$resource->identifier);
            }
        }
        ...
        $mform->setDefault('name', stripslashes_safe($sharedresource_entry->title));
        $mform->setDefault('description', stripslashes_safe($sharedresource_entry->description));
        
        $location = $mform->addElement('static', 'origtitle', get_string('title', 'sharedresource').': ', stripslashes_safe($sharedresource_entry->title));
...
//              . "'resource{$sharedresource_entry->id}','resizable=1,scrollbars=1,directories=1,location=0,menubar=0,toolbar=0,status=1,width=800,height=600');\">(".$strpreview.")</a>";
        ...
        $buttonattributes = array('title'=> get_string('searchsharedresource', 'sharedresource'), 'onclick'=>" window.location.href ='"
                          . $CFG->wwwroot."/mod/sharedresource/search.php?course={$this->course->id}&section={$section}&type={$type}&add={$add}&return={$return}&insertinpage={$insertinpage}"."'; return false;");
