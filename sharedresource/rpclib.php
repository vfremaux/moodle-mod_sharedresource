...
function sharedresource_rpc_get_metadata($remoteuser, $remoteuserhost, $element, $namespace = 'lom') {
    ...
    // Get local identity
    $userhost = get_record('mnet_host', 'wwwroot', $remoteuserhost);
    if (!$localuser = get_record('user', 'username', addslashes($remoteuser), 'mnethostid', $userhost->id)){
...
function sharedresource_rpc_get_list($remoteuser, $remoteuserhost, $metadatafilters = '', $offset = 0, $page = 20) {
    ...
    if (empty($metadatafilters)){
        ...
        $entrycount = count_records_sql($sqlcount);
        $response->resources['maxobjects'] = $entrycount;
        
        // debug_trace('without filters. >> '.$sql);
        $entries = get_records_sql($sql, $offset, $page);
    } else {
    	// we have filters
	...
	if ($entries){
        foreach($entries as $entry){
            ...
            $response->resources['entries'][$entry->identifier] = array('title' => $entry->title,
                                                           ...
                                                           'uses' => $uses);
            // get all metadata
            if ($metadata = get_records('sharedresource_metadata', 'entry_id', $entry->id, 'element', 'element,namespace,value')){
...
function sharedresource_rpc_submit($remoteuser, $remoteuserhost, &$entry, $metadata){
	...
	if (!is_array($entry) && !is_object($entry)){
		...
		// an exiting resource 
	    $entry = (object) $entry;
	    // if (!isset($entry->url)) print_object($entry);
	    $oldurl = $entry->url;
	    ...
	    if(!$localentry = get_record('sharedresource_entry', 'identifier', $entry->identifier)){
	        $newid = insert_record('sharedresource_entry', $entry);
	    } else {
	        $entry->id = $localentry->id;
	        $newid = update_record('sharedresource_entry', $entry);
	    }
    ...
    if (!empty($metadata)){
	    delete_records('sharedresource_metadata', 'entry_id', $newid); // for replacing old metadata by submitted one. May not have any records in case of a new resource.
	    foreach($metadata as $datum){
	        $datum->entry_id = $newid;
	        insert_record('sharedresource_metadata', $datum);
        }
	}
...
function sharedresource_rpc_check($remoteuser, $remoteuserhost, $resourceID){
    ...
    $uses = count_records('sharedresource', 'identifier', $resourceID);
    return $uses;        
}
...
function sharedresource_rpc_move($remoteuser, $remoteuserhost, $resourceID, $provider, $url){
    ...
    if ($resource_entry = get_record('sharedresource_entry', 'identifier', $resourceID)){
        $resource_entry->url = $url;
        $resource_entry->provider = $provider;
        if (update_record('sharedresource_entry', $resource_entry)){
            $response->status = RPC_SUCCESS;
        } else {
            $response->status = RPC_FAILURE;
