	...
	if($mode == 'delete' && !empty($target)){
		...
		// when a classification is deleted
			...
			foreach($plugins as $num => $plugin){
				...
				$elementinstances = get_records_select('sharedresource_metadata', "namespace = '{$plugin}' and element LIKE '{$numclassification}%' and value = '{$target}'" );
				...
				if(!empty($elementinstances)){
					...
					foreach($tabinstance as $key => $value){ // Metadata which contain elements of this classification are deleted
						$elementinstances2 = delete_records_select('sharedresource_metadata', "namespace = '{$plugin}' and element LIKE '{$numclassification}%:{$value}%'" );
					}
				}
