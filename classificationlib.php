...
function metadata_create_classification($classtable,$classifarray,$classification){
	...
	$finalclassif = metadata_create_classification_iter($tempclassif,$newclassif,$classifarray,$classification);
	return $finalclassif;
}

function metadata_create_classification_iter($tempclassif, $newclassif, $classifarray, $classification){
	...
	foreach($classifarray as $name => $infos){
		if($infos['select'] == 1){
			if($infos['restriction'] == ''){
				$classtable = get_records($name);
			} else {
				$classtable = get_records_sql($classifarray[$name]['restriction']);
			}
			$finalclassif = metadata_create_classification($classtable,$classifarray,$name);
			echo '<option style="font-weight:bold;text-align:center;" disabled="disabled" value="'.$name.'">'.$name.'</option>';
...
function print_classif2($classifarray){
	if (!empty($classifarray)){
		foreach($classifarray as $name => $infos){
			if($infos['select'] == 1){
				echo '<option style="font-weight:bold;text-align:center;" value="'.$name.'">'.$infos['classname'].'</option>';
			}
		}
	}
}
	...
	//if we are searching the childs of a taxon
		...
		if(!empty($classif)){
			if($classifarray[$classif]['restriction'] == ''){
				$classtable = get_records($classif);
			} else {
				$classtable = get_records_sql($classifarray[$classif]['restriction']);
			}
