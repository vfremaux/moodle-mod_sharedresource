...
function metadara_create_tab($capability, &$mtdstandard) {
	...
	for ($i = 1; $i <= $nbrmenu; $i++) {
		...
		if($DB->record_exists_select('config_plugins', "name LIKE 'config_{$mtdstandard->pluginname}_{$capability}_{$i}'") == true){
			echo 'inline;">'."\n\t\t";
		} else {
			echo 'none;">'."\n\t\t";
		}
				...
				if($fill == ''){
					...
					foreach($mtdstandard->METADATATREE[$fieldnum]['values'] as $value) {
						if (is_number($value)){
							...
							$str = get_string(clean_string_key($value), 'sharedresource');
...
function metadata_get_children_nodes(&$mtdstandard, $fieldnum, $capability, $listchildren = array()){
	...
	for ($i = 1; $i <= $childcount; $i++) {
		$currentfield = $fieldnum.'_'.$i;
		if ($DB->record_exists_select('config_plugins', "name LIKE 'config_{$mtdstandard->pluginname}_{$capability}_{$currentfield}'") == true){
