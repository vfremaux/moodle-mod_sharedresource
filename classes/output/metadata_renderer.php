...
class metadata_renderer extends plugin_renderer_base {
    ...
    public function metadata_configuration() {
        ...
        if (!empty($config->schema)) {
            ...
            $template->backadminpageurl = new moodle_url('/admin/settings.php', ['section' => 'modsettingsharedresource']);
        } else {
            $template->noplugin = $this->output->notification(get_string('nometadataplugin', 'sharedresource'));
        }

        $template->defaultconfigurl = new moodle_url('/mod/sharedresource/metadataconfigure.php', ['action' => 'reinitialize']);
        $template->settingsurl = new moodle_url('/admin/settings.php', ['section' => 'modsettingsharedresource']);
     ...
     * @param string $capability the users capability against sharedresource metadatas
     */
    public function notice($shrentry, $capability) {
        ...
        $childs = $mtdstandard->get_element_childs(0);
        ...
        $template->standarddescriptionstr = get_string('standarddescription', 'sharedmetadata_'.$mtdstandard->get_namespace());
        ...
        foreach ($childs as $nodeid => $islist) {
            ...
            if ($paneltpl->hascontent) {
                ...
                $template->tabs[] = $this->tab($i, $nodeid, $capability, 'read');
            }
            $i++;
        }
     ...
     */
    protected function tab($i, $nodeid, $capability, $mode = 'read') {
        ...
        if (metadata::use_branch($nodeid, $capability, $mode)) {
     ...
     * @param boolean $realoccur is used only in the case of classification, when a classification is deleted by an admin
     * and does not appear anymore on the metadata notice.
     */
    public function part_view(&$parenttemplate, &$shrentry, $elementkey, $capability, $realoccur = 0) {
        ...
        if (!$mtdstandard->has_node($nodeid)) {
        ...
        if (optional_param('debug', false, PARAM_BOOL) && ($CFG->debug >= DEBUG_NORMAL)) {
            ...
            echo "Resource index : ".$mtdstandard->is_resource_index($nodeid)."<br/>";
            echo "<br/>";
        }
        ...
        if ($standardelm->type == 'category') {
            ...
            if (!empty($taxumarray) && $nodeid == $taxumarray['main']) {
                ...
                $standardelmchilds = $mtdstandard->get_element_childs($nodeid);
            ...
            if ($numoccur == 0 && $standardelm->islist) {
                ...
                if (!empty($siblings)) {
                    ...
                    if ($mtdstandard->is_resource_index($nodeid)) {
     ...
     * @param object $standardelm The metadata element's definition in the standard
     * @param object $elminstance The metadata effective instance with value inside
    ...
    public function print_data($standardelm, $elminstance, &$template) {
        ...
        $sizeelement = $mtdstandard->get_size_element();
        ...
        switch ($standardelm->type) {
            ...
            case 'codetext': {
                ...
                $value = $elminstance->get_value();
                if ($elminstance->get_node_id() == $mtdstandard->get_title_element()->name) {
            ...
            case 'date': {
                ...
                if (!empty($value)) {
                    ...
                    if (!is_array($value)) {
                        ...
                        foreach ($value as $item) {
                            $cleanedkey = clean_string_key($item);
                            $dates[] = date(get_string('datefmt', 'sharedresource'), $item);
                        }
            ...
            case 'duration': {
                ...
                if ($value != '') {
                    $template->hascontent = true;
                    $time = metadata::build_time($value);
                ...
                $template->mtdvalue .= $this->output->help_icon('durationdescr', 'sharedresource', $duration);
                break;
            }
     ...
     */
    public function metadata_edit_form($capability, $mtdstandard, $shrentry) {
        global $SESSION;

        // Get context params in.
        // Return to course or library.
        $fromlibrary = optional_param('fromlibrary', 1, PARAM_BOOL);
        // Return to course/view.php if false or mod/modname/view.php if course, 'browse' or 'explore' if library.
    ...
    public function edit_panels($capability, $mtdstandard, & $template) {
        ...
        $rootnodes = $mtdstandard->get_element_childs(0);
        ...
        foreach ($rootnodes as $rootid => $islist) {
            ...
            $mode = 'write';
            $template->tabs[] = $this->tab($rootid, $rootid, $capability, $mode);
     ...
     * @param boolean $realoccur is used only in the case of classification, when a classification is deleted by an admin
     * and does not appear anymore on the metadata form. Realoccur is the visible occurrence the user should see for the part.
     * @param bool $ispanel
     */
    public function part_form(&$parenttemplate, $elementkey, $capability, $realoccur = 0, $ispanel = false) {
        ...
        mod_sharedresource_debug_trace($debug, SHR_TRACE_DEBUG_FINE);
        mod_sharedresource_debug_trace($elminstance, SHR_TRACE_DEBUG_FINE);
        mod_sharedresource_debug_trace($standardelm, SHR_TRACE_DEBUG_FINE);
        ...
        if ($standardelm->type == 'category') {
            if (!is_null($taxumarray) && ($nodeid == $taxumarray['main'])) {
                ...
                if (function_exists('debug_trace')) {
                    mod_sharedresource_debug_trace('Processing taxonomy element', SHR_TRACE_DEBUG);
                }
                ...
                $sourceelminstancekey = metadata::to_instance($sourceelmnodeid, $elminstance->get_instance_id());
                $sourceelm = metadata::instance($shrentry->id, $sourceelminstancekey, $namespace, false);
                $taxonelm = $sourceelm->get_parent(false); // May not really exist in DB.

                $sourcevalue = $sourceelm->get_value();
                if (empty($sourcevalue)) {
                    mod_sharedresource_debug_trace(" Taxon Source initial value is undefined", SHR_TRACE_ERRORS);
                    ...
                    if ($instanceindex == 0) {
                         ...
                         */
                        mod_sharedresource_debug_trace(" Taxon Source initial : Take first one", SHR_TRACE_DEBUG);
                        ...
                        if ($sourceinstance->isstored) {
                            $params = ['id' => $sourceinstance->get_value()];
                ...
                if (empty($sourceelmid)) {
                    ...
                    $attrs = ['class' => 'mtd-form-input', 'id' => $template->elmname, 'data-source' => $htmlsourcekey];
                    $nochoice = ['' => get_string('none', 'sharedresource')];
                    ...
                    mod_sharedresource_debug_trace($sourceselect, SHR_TRACE_DEBUG_FINE);
                    $taxontpl->classificationselect = $sourceselect;
                    $taxontpl->occur = $template->occur;
                    $template->taxons[] = $taxontpl;

                    /*
                     * Get all other taxons that may be added to this classification set. This will shift the apparent
                     * $addelementkey pushing it up for any available taxon sibling.
                     */
                    if ($value) {
                        /*
                        ...
                        foreach ($siblings as $sib) {
                            ...
                            $attrs = ['class' => 'mtd-form-input', 'id' => $htmlname, 'data-source' => $htmlsourcekey];
                            $taxontpl->classificationselect = html_writer::select($classificationoptions, $htmlname,
                                    $sib->get_value(),
                                    $nochoice, $attrs);
                            $taxontpl->occur = $i;
                            $template->taxons[] = $taxontpl;
                ...
                if ($elminstance->isstored) {
                    ...
                    $standardchilds = $mtdstandard->get_element_childs($nodeid, $capability, 'write', true);
                    $listresults = [];
        ...
        if (!isset($siblingcollector)) {
            ...
            if (( ((integer) $numoccur) === 0)) {
                ...
                mod_sharedresource_debug_trace("START Element siblings for ".$elminstance->get_element_key(), SHR_TRACE_DEBUG_FINE);
                if (!empty($siblings)) {
                    // All siblings will have a numoccur > 0.
                    $i = 1;
                    foreach ($siblings as $sib) {
                        $msg = "Calling form for element sibling ".$sib->get_element_key();
                        mod_sharedresource_debug_trace($msg, SHR_TRACE_DEBUG_FINE);
                        $this->part_form($siblingcollector, $sib->get_element_key(), $capability, $i);
                        mod_sharedresource_debug_trace("Call out", SHR_TRACE_DEBUG_FINE);
                        $i++;
                    }
                    // Reajust maxoccur on last numoccur.
                    $lastoccur = $i;
                }
                mod_sharedresource_debug_trace("END Element siblings", SHR_TRACE_DEBUG_FINE);
            }
        }

        $template->hasaddbutton = false;
        if ($standardelm->islist) {
            $msg = "Realoccur:{$realoccur};LastOccur:{$lastoccur};MaxOccur:".($elminstance->maxoccur ?? 0);
            $msg .= ";IsAjaxRoot:".@$parenttemplate->is_ajax_root;
            mod_sharedresource_debug_trace($msg, SHR_TRACE_DEBUG_FINE);
            $template->debugdata .= $msg;
            ...
            if ($printaddbutton) {
                mod_sharedresource_debug_trace("PrintAddButton", SHR_TRACE_DEBUG_FINE);
                 ...
                 *
                 * If $lastoccur is really empty, the form is a "new element form", so disable the button, untill the
                 * value is changed.
                 */
        ...
        mod_sharedresource_debug_trace("<<<<\n", SHR_TRACE_DEBUG);
        $traceindentlevel--;
        $CFG->traceindent = str_pad(' ', $traceindentlevel * 4);
        return $template;
    }
    ...
    /**
     * Print a single metadata widget.
     * @param object $mtdstandard
     * @param object $elminstance
     * @param object &$standardelm
     * @param object $template
     * @param object $shrentry
     */
    protected function print_widget($mtdstandard, $elminstance, $standardelm, $template, $shrentry) {
        ...
        if ($standardelm->type == 'text' ||
            ...
            if ($nodeid == $mtdstandard->get_title_element()->node) {
            ...
            } else if ($nodeid == $mtdstandard->get_description_element()->node) {
                if ($elminstance->get_element_key() == $firstelmkey) {
            ...
            $attrs = ['id' => $template->elmname, 'class' => ' mtd-form-input'];
            if (!empty($mtdstandard->metadatatree[$nodeid]['extraclass'])) {
                $attrs['class'] .= ' '.$mtdstandard->metadatatree[$nodeid]['extraclass'];
            }
            $default = ['' => get_string('none', 'sharedresource')];
            $current = $elminstance->get_value();
            if (empty($options)) {
                $template->select = $this->output->notification("Missing options for $template->elmname in standard plugin");
            } else {
            ...
            list($fillyear, $fillmonth, $fillday) = ['', '', ''];
            if ($elminstance->get_value() != '') {
            ...
            $attrs = ['id' => $template->elmname.'_dateyear', 'class' => 'mtd-form-date-select'];
            $template->yearselect = html_writer::select($options, $template->elmname.'_dateyear', $fillyear, '', [], $attrs);
            ...
            $attrs = ['id' => $template->elmname.'_datemonth', 'class' => 'mtd-form-date-select'];
            $template->monthselect = html_writer::select($options, $template->elmname.'_datemonth', $fillmonth, '', [], $attrs);
            ...
            $attrs = ['id' => $template->elmname.'_dateday', 'class' => 'mtd-form-date-select'];
            $template->dayselect = html_writer::select($options, $template->elmname.'_dateday', $fillday, '', [], $attrs);
            ...
            if ($elminstance->get_value() != '') {
                $time = metadata::build_time($elminstance->get_value());
