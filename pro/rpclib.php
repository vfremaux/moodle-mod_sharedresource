...
/**
 * Interface : provider
 * retrieve all entries from a metadata element
 ...
 */
function sharedresource_rpc_get_list($remoteuser, $remoteuserhost, $remotehost, $metadatafilters = '', $offset = 0, $page = 20) {
    ...
    $localconfig = get_config('local_sharedresources');
    $fs = get_file_storage();

    // Get local identity for tracing if needed.

    $userhost = $DB->get_record('mnet_host', array('wwwroot' => $remoteuserhost));
    if (!$localuser = $DB->get_record('user', array('username' => $remoteuser, 'mnethostid' => $userhost->id))) {
        $localuser = $DB->get_record('user', array('username' => 'guest'));
    }
    $USER = $localuser;

    // Check the call comes from a legitimate consumer.
    if (!empty($localconfig->privatecatalog)) {
        if (!sharedresources_is_consumer($remotehost)) {
            $response->status = RPC_ERROR;
            $response->error = "Not an allowed consumer.";
            return json_encode($response);
        }
    }

    if (empty($metadatafilters)) {
        debug_trace($CFG->wwwroot.": Getting without filters ");
        $sql = "
            SELECT
                *
            FROM
        ...
        $entrycount = $DB->count_records_sql($sqlcount, array($systemcontext->id));
        $response->resources['maxobjects'] = $entrycount;
        $entries = $DB->get_records_sql($sql, array($systemcontext->id), $offset, $page);
    } else {
        // We have filters.
        debug_trace($CFG->wwwroot.": Getting with filters ");
        debug_trace($metadatafilters);
        ...
        foreach ($mtdfiltersarr as $filterkey => $filtervalue) {
            if (!empty($filtervalue)) {
                ...
                $entrysets = sharedresource_get_by_metadata($filterkey, $config->schema, 'entries', $filtervalue);
        ...
        if (!empty($mtdrecs)) {
            list($clausesql, $clauseparams) = $DB->get_in_or_equal(array_values($mtdrecs));
            $entrylist = implode("','", array_values($mtdrecs));
            $clause = ' se.id '.$clausesql;
            $sql = "
                SELECT
                    se.*
                FROM
            ...
            $entries = $DB->get_records_sql($sql, $clauseparams, $offset, $page);
        } else {
            $entries = array();
        }
    }
    ...
    if ($entries) {
        ...
        // If library is private, we need give the consumer an access token to pass security traps.
        $token = '';
        if ($localconfig->privatecatalog) {
            include_once($CFG->dirroot.'/auth/ticket/lib.php');
            $token = ticket_generate($localuser, 'sharedresources_access', null, 'internal', 'persistance');
        }
        ...
        foreach ($entries as $entry) {
            ...
            $resourcedesc = array('title' => $entry->title,
                ...
                'lang' => $entry->lang,
                'isurlproxy' => empty($entry->file), // This is a local file id in the local repo.
                'scorelike' => $entry->scorelike,
                'scoreview' => $entry->scoreview,
                'id' => empty($entry->id),
                'uses' => $uses,
                'provider' => $CFG->wwwroot,
                'token' => $token);

            // Fetch explicit file information.
            if (!empty($entry->file)) {
                if ($filerec = $fs->get_file_by_id($entry->file)) {
                    $resourcedesc['file_filepath'] = $filerec->get_filepath();
                    $resourcedesc['file_filename'] = $filerec->get_filename();
                    $resourcedesc['file_mimetype'] = $filerec->get_mimetype();
                }
            }

            // Locally checks typology to report it on the consumer side.
            if ($resourcedesc['islti'] = sharedresource_is_lti($entry)) {
                $resourcedesc['file_mimetype'] = 'application/lti';
            }
            $resourcedesc['ismedia'] = sharedresource_is_media($entry);
            $resourcedesc['isdeployable'] = sharedresource_is_moodle_activity($entry);
            $resourcedesc['isscorm'] = sharedresource_is_scorm($entry);

            // Fetch file image representation.
            $customresourcethumbs = $fs->get_area_files($systemcontext->id, 'mod_sharedresource', 'thumbnail', $entry->id, '', false);
            if (!empty($customresourcethumbs)) {
                $customthumbfile = array_pop($customresourcethumbs);
                $resourcedesc['file_iconurl'] = ''.moodle_url::make_pluginfile_url($systemcontext->id, 'mod_sharedresource',
                                                                                'thumbnail', $entry->id,
                                                                                $customthumbfile->get_filepath(),
                                                                                $customthumbfile->get_filename(), false);
            } else {
                if ($resourcedesc['isdeployable']) {
                    $resourcedesc['file_iconurl'] = sharedresource_extract_activity_icon($resourcedesc);
                } else {
                    if (!empty($filerec)) {
                        $resourcedesc['file_iconurl'] = ''.$OUTPUT->image_url(file_file_icon($filerec, 128));
                    } else {
                        $resourcedesc['file_iconurl'] = ''.$OUTPUT->image_url('weblink', 'local_sharedresource');
                    }
                }
            }

            $response->resources['entries'][$entry->identifier] = $resourcedesc;
            // Get all metadata.
...
/**
 * Interface : provider
 * retrieve the remote list of search widgets
 * @param string $remoteuser the username of the remote user
 * @param string $remoteuserhost the MNET hostname of the remote user
 * @param array $context a user sharedresource related context, regarding local sharedrerources capabilities.
 * @param int $searchvalues some intial values (possibly comming from form).
 */
function sharedresource_rpc_get_widgets($remoteuser, $remoteuserhost, $remotehost, $context = '', $searchvalues = array()) {
    global $DB, $USER, $CFG;

    debug_trace("Getting search widgets on ".$CFG->wwwroot);
    ...
    if (!empty($localconfig->privatecatalog)) {
        if (!sharedresources_is_consumer($remotehost)) {
            $response->status = RPC_ERROR;
            $response->error = "$remotehost is not an allowed consumer.";
            debug_trace("Not allowed consumer $remotehost");
            return json_encode($response);
        }
    }

    sharedresources_setup_widgets($visiblewidgets, $context);
    $response->widgets = $visiblewidgets;
